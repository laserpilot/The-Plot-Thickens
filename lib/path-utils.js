...
const { DensityField } = require('./density-field');
 ...
 * @param {number} outlineOffset - Offset distance for outline thickness passes in mm (default: 0.25)
 * @param {number} outlinePasses - Number of passes for outline thickness (default: 1)
...
function generatePasses(pathData, passes, baseOffset, noise, seed = null, pathId = '', offsetEnvelope = null, useNormalMode = false, noiseFrequency = 50, sampleRate = 2, fillMode = 'offset', crosshatchOptions = null, extractOutline = false, stripeFilled = 1, stripeEmpty = 1, outlineOffset = 0.25, outlinePasses = 1) {
  ...
  if (fillMode === 'crosshatch' && crosshatchOptions) {
    ...
    return generateCrosshatchFill(pathData, baseWidth, angles, spacing, noise, seed, pathId, offsetEnvelope, noiseFrequency, sampleRate, organic, extractOutline, outlineOffset, outlinePasses);
  }
  ...
  if (fillMode === 'stippling' && crosshatchOptions) {
    ...
    const baseWidth = baseOffset * Math.max(1, passes);
    return generateStipplingFill(pathData, baseWidth, dotSpacing, dotSize, seed, pathId, offsetEnvelope, sampleRate, extractOutline, outlineOffset, outlinePasses);
  }
  ...
  if (fillMode === 'hatch-gradient' && crosshatchOptions) {
    ...
    return generateHatchGradientFill(
      ...
      lightMode, lightPosX, lightPosY, falloffRadius, outlineOffset, outlinePasses
    );
  }
  ...
  if (fillMode === 'focus-blur' && crosshatchOptions) {
    ...
    // If outlines requested, generate multiple thickness passes for outlines
    if (extractOutline && effectivePasses > 0) {
      const rightPassIndex = Math.floor((effectivePasses - 1) / 2);
      const rightBaseDistance = rightPassIndex * baseOffset;

      for (let i = 0; i < outlinePasses; i++) {
        const rightDistance = rightBaseDistance + (i * outlineOffset);
        const rightOutline = offsetPathWithNoiseField(pathData, rightDistance, seed + effectivePasses + i * 100, pathId, offsetEnvelope, sampleRate, densityField, noiseFieldParams);
        outlinePathsArray.push(rightOutline);
      }

      if (effectivePasses > 1) {
        const leftPassIndex = Math.floor((effectivePasses - 2) / 2) + 1;
        const leftBaseDistance = -leftPassIndex * baseOffset;

        for (let i = 0; i < outlinePasses; i++) {
          const leftDistance = leftBaseDistance - (i * outlineOffset);
          const leftOutline = offsetPathWithNoiseField(pathData, leftDistance, seed + effectivePasses + 1 + i * 100, pathId, offsetEnvelope, sampleRate, densityField, noiseFieldParams);
  ...
  if (extractOutline && passes > 0) {
    // Generate right outline (furthest right) with multiple thickness passes
    const rightPassIndex = Math.floor((passes - 1) / 2);
    const rightBaseDistance = rightPassIndex * baseOffset;

    for (let i = 0; i < outlinePasses; i++) {
      const rightDistance = rightBaseDistance + (i * outlineOffset);
      const rightOutline = offsetPath(pathData, rightDistance, noise, seed + passes + i * 100, pathId, offsetEnvelope, useNormalMode, noiseFrequency, sampleRate);
      outlinePathsArray.push(rightOutline);
    }

    // Generate left outline (furthest left) with multiple thickness passes if we have multiple passes
    if (passes > 1) {
      ...
      for (let i = 0; i < outlinePasses; i++) {
        const leftDistance = leftBaseDistance - (i * outlineOffset);
        const leftOutline = offsetPath(pathData, leftDistance, noise, seed + passes + 1 + i * 100, pathId, offsetEnvelope, useNormalMode, noiseFrequency, sampleRate);
        outlinePathsArray.push(leftOutline);
      }
    }
  }
 ...
 */
function generateCrosshatchFill(pathData, baseWidth, hatchAngles, hatchSpacing, noise = 0, seed = 0, pathId = '', offsetEnvelope = null, noiseFrequency = 50, sampleRate = 2, organicOptions = {}, extractOutline = false, outlineOffset = 0.25, outlinePasses = 1) {
 ...
 */
function generateStipplingFill(pathData, baseWidth, dotSpacing, dotSize, seed = 0, pathId = '', offsetEnvelope = null, sampleRate = 2, extractOutline = false, outlineOffset = 0.25, outlinePasses = 1) {
  try {
    const absolutePath = pathToAbsolute(pathData);
    const totalLength = getTotalLength(absolutePath);
