      ...
      while (currentArcLength <= totalLength) {
        ...
        if (lightMode === 'point') {
          ...
          if (dist < 0.001) {
            ...
            // Orientation weight: surfaces facing away from light get denser
            const orientWeight = (1 - Math.abs(orientDot)) * 0.5; // [0, 0.5]

            // Distance falloff: further from light = denser
            // Falloff radius is where light drops to 50%
            const distanceFalloff = 1 / (1 + dist / falloffRadius);
            const distWeight = (1 - distanceFalloff) * 0.5; // [0, 0.5]

            // Combine both contributions
            rawWeight = (orientWeight + distWeight) * lightStrength + baseWeight;
          }
        } else {
          // DIRECTIONAL MODE: Global light direction (current implementation)
          ...
          // Map dot product [-1, 1] to weight [baseWeight, 1]
