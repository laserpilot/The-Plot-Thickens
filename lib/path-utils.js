 ...
 */
function offsetPathLegacy(pathData, offset, noise = 0, seed = 0) {
  try {
 ...
 * @returns {string} Offset path data
 */
function offsetPathNormal(pathData, offset, noise = 0, seed = 0, pathId = '', offsetEnvelope = null, noiseFrequency = 50) {
  try {
    // Skip expensive resampling if offset and noise are both zero
    if (offset === 0 && noise === 0) {
      return pathData;
 ...
 * @returns {string} Offset path data
 */
function offsetPath(pathData, offset, noise = 0, seed = 0, pathId = '', offsetEnvelope = null, useNormalMode = false, noiseFrequency = 50) {
  if (useNormalMode) {
    return offsetPathNormal(pathData, offset, noise, seed, pathId, offsetEnvelope, noiseFrequency);
  } else {
    return offsetPathLegacy(pathData, offset, noise, seed);
  }
}
 ...
 */
function generatePasses(pathData, passes, baseOffset, noise, seed = null, pathId = '', offsetEnvelope = null, useNormalMode = false, noiseFrequency = 50) {
  const paths = [];
  ...
  for (let i = 0; i < passes; i++) {
    ...
    const offsetPathData = offsetPath(pathData, offsetDistance, noise, passSeed, pathId, offsetEnvelope, useNormalMode, noiseFrequency);
