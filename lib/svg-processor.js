...
function processPaths(svgData, config, stream = null) {
  ...
  lengths.forEach(({ path, length }, index) => {
    ...
    if (fillMode === 'crosshatch') {
      ...
      // Convert % light position to user units for point mode
      const viewBox = svgData.viewBox || { x: 0, y: 0, width: 100, height: 100 };
      const lightPosXUser = (config.focusBlur.lightPosX / 100) * viewBox.width;
      const lightPosYUser = (config.focusBlur.lightPosY / 100) * viewBox.height;
      ...
      fillModeOptions = {
        ...
        viewBox: viewBox // Pass viewBox for density field initialization
      };
    }
