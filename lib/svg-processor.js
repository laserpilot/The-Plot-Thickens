...
const fs = require('fs');
const { measurePathLength, generatePasses, lengthToWeight, getEnvelopePreset, generateShapeFill } = require('./path-utils');
...
function processPaths(svgData, config, stream = null) {
  ...
  lengths.forEach(({ path, length }, index) => {
    ...
    if (fillMode === 'crosshatch') {
      ...
      let viewBoxObj = { x: 0, y: 0, width: 100, height: 100 };
      if (svgData.viewBox) {
        const parts = svgData.viewBox.split(/\s+/).map(parseFloat);
        if (parts.length === 4) {
          viewBoxObj = { x: parts[0], y: parts[1], width: parts[2], height: parts[3] };
        }
      }

      // Convert % light position to user units for point mode
      const lightPosXUser = (config.hatchGradient.lightPosX / 100) * viewBoxObj.width;
      const lightPosYUser = (config.hatchGradient.lightPosY / 100) * viewBoxObj.height;
    ...
    } else if (fillMode === 'focus-blur' && config.focusBlur) {
      // Convert focus-blur config to options format expected by generatePasses
      const lightMode = config.focusBlur.lightMode || 'directional';
      ...
      // Convert % light position to user units for point mode
      const lightPosXUser = (config.focusBlur.lightPosX / 100) * viewBoxObj.width;
      const lightPosYUser = (config.focusBlur.lightPosY / 100) * viewBoxObj.height;

      fillModeOptions = {
        lightMode: lightMode,
        lightAngle: config.focusBlur.lightAngle || 45,
        lightPosX: lightPosXUser,
        lightPosY: lightPosYUser,
        falloffRadius: config.focusBlur.falloffRadius || 150,
        noiseMin: config.focusBlur.noiseMin || 0.05,
        noiseMax: config.focusBlur.noiseMax || 0.6,
        freqMin: config.focusBlur.freqMin || 100,
        freqMax: config.focusBlur.freqMax || 10,
        modulatePasses: config.focusBlur.modulatePasses || false,
        passesMin: config.focusBlur.passesMin || 1.0,
        passesMax: config.focusBlur.passesMax || 1.5,
        viewBox: viewBoxObj // Pass parsed viewBox for density field initialization
      };
    } else if (fillMode === 'spiral') {
      // Convert spiral config to options format (reusing crosshatchOptions mechanism)
      fillModeOptions = {
        twistRate: config.spiralTwistRate || 0.01, // radians per mm
        twistOffset: (config.spiralTwistOffset || 0) * (Math.PI / 180) // convert degrees to radians
      };
    }

    const extractOutline = config.addOutline || false;

    const stripeFilled = (fillMode === 'striped' || fillMode === 'spiral') ? (config.stripeFilled || 1) : 1;
    const stripeEmpty = (fillMode === 'striped' || fillMode === 'spiral') ? (config.stripeEmpty || 1) : 1;

    // Handle shape-fill mode separately (doesn't use generatePasses)
    let result;
    if (fillMode === 'shape-fill') {
      result = generateShapeFill(path.d, {
      ...
      // Traditional offset mode with generatePasses
      result = generatePasses(
      path.d,
      passes,
      config.baseOffset,
      config.noise,
      ...
      config.outlineOffset || 0.25, // Outline thickness offset
      config.outlinePasses || 1 // Number of outline thickness passes
      );
    }
