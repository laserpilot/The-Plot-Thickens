...
function processPaths(svgData, config, stream = null) {
  ...
  lengths.forEach(({ path, length }, index) => {
    ...
    const crosshatchOptions = config.crosshatch || null;
    ...
    const result = generatePasses(
      ...
      fillMode, // Fill mode: 'offset' or 'crosshatch'
      crosshatchOptions, // Crosshatch options: {angles, spacing}
    ...
    if (bins && binBoundaries) {
      ...
      outlines.forEach((outlinePath) => {
        stream.write(`    <path d="${outlinePath}" fill="none" stroke="${path.stroke}" stroke-width="${path.strokeWidth}" class="outline" />\n`);
        totalOutputPaths++;
      });
    }
  });
...
function writeBinnedOutput(stream, bins, boundaries) {
  bins.forEach((bin, binIndex) => {
    ...
    bin.forEach(({ duplicates, outlines, path }) => {
      ...
      // Write outline paths if present
