  ...
  for (let chunkStart = 0; chunkStart < totalPaths; chunkStart += chunkSize) {
    ...
    if (config.fillMode === 'barber-pole') {
      const barberPolePaths = generateBarberPoleFill(path.d, {
        ...
        stripeThickness: config.stripeThickness !== undefined ? config.stripeThickness : null,
        stripeGapRatio: config.stripeGapRatio !== undefined ? config.stripeGapRatio : 1.0,
        stripeLineSpacing: config.stripeLineSpacing !== undefined ? config.stripeLineSpacing : 0.3,
