 ...
 */
export function buildSVG(paths, bounds, metadata = {}, enableBinning = false, binCount = 4, originalPaths = [], originalSvgMetadata = {}, outputSize = 'original') {
  ...
  if (enableBinning && originalPaths.length > 0) {
    return buildBinnedSVG(paths, bounds, metadata, binCount, originalPaths, originalSvgMetadata, outputSize);
  }
 ...
 */
function buildBinnedSVG(paths, bounds, metadata, binCount, originalPaths, originalSvgMetadata = {}, outputSize = 'original') {
  const parts = [];
  const { x = 0, y = 0, width, height } = bounds;
