 ...
 */
export function buildSVG(paths, bounds, metadata = {}, enableBinning = false, binCount = 4, originalPaths = [], originalSvgMetadata = {}, outputSize = 'original', fullConfig = null, preserveLayers = false) {
  ...
  // Check if layer preservation is enabled and paths have layer info
  const hasLayers = preserveLayers && paths.some(p => p.layerId);
  if (hasLayers) {
    return buildLayeredSVG(paths, bounds, metadata, originalSvgMetadata, outputSize, fullConfig);
  }
...
/**
 * Build SVG with layer-first grouping (preserves input layer structure)
 * Hierarchy: layer > family/fills/outlines
 * @param {Array} paths - Processed paths with layerId property
 * @param {Object} bounds - SVG bounds
 * @param {Object} metadata - Metadata
 * @param {Object} originalSvgMetadata - Original SVG metadata
 * @param {string} outputSize - Output size override
 * @param {Object} fullConfig - Complete config object to embed in SVG metadata
 * @returns {string} SVG string with layer groups
 */
function buildLayeredSVG(paths, bounds, metadata, originalSvgMetadata = {}, outputSize = 'original', fullConfig = null) {
  const parts = [];
  const { x = 0, y = 0, width, height } = bounds;
  ...
  if (fullConfig) {
    parts.push('  <metadata id="plotter-config">');
    parts.push('    <config xmlns="https://github.com/laserpilot/The-Plot-Thickens">');
    const configJson = JSON.stringify(fullConfig, null, 6);
    parts.push(configJson.split('\n').map(line => '      ' + line).join('\n'));
    parts.push('    </config>');
    parts.push('  </metadata>');
  }
  ...
  const fillMode = metadata.fillMode || 'offset';
  const timestamp = new Date().toISOString();
  parts.push(`  <!-- Generated by The Plot Thickens -->`);
  parts.push(`  <!-- Fill mode: ${fillMode}, Layers preserved -->`);
  parts.push(`  <!-- Generated: ${timestamp} -->`);
  ...
  paths.forEach(path => {
    if (path.layerId) {
      if (!layerGroups[path.layerId]) {
        layerGroups[path.layerId] = [];
      }
      layerGroups[path.layerId].push(path);
    } else {
      ungroupedPaths.push(path);
    }
  });

  // Output each layer group
  const layerNames = Object.keys(layerGroups).sort();

  layerNames.forEach(layerName => {
    const layerPaths = layerGroups[layerName];
    const safeId = layerName.replace(/[^a-zA-Z0-9-_]/g, '_');

    parts.push(`  <g id="layer-${safeId}" data-original-layer="${escapeXML(layerName)}">`);
    parts.push(`    <!-- Layer: ${escapeXML(layerName)} (${layerPaths.length} paths) -->`);

    // Within each layer, apply existing grouping logic (family/fill/outline)
    const hasFamilies = layerPaths.some(p => p.family);
    const fillPaths = layerPaths.filter(p => !p.isOutline);
    const outlinePaths = layerPaths.filter(p => p.isOutline);

    if (hasFamilies) {
      // Group by family within layer
      const familyGroups = {};
      fillPaths.forEach(path => {
        const family = path.family || 'default';
      ...
      Object.keys(familyGroups).sort().forEach(family => {
        ...
        parts.push(`    <g id="layer-${safeId}-family-${family}" stroke="${stroke}">`);
        parts.push(`      <!-- ${family} stripes (${familyPaths.length} paths) -->`);
        familyPaths.forEach(path => {
          if (!path.d) return;
          const escapedD = escapeXML(path.d);
          parts.push(`      <path d="${escapedD}" fill="none" stroke-width="${path.strokeWidth || 0.1}" />`);
        });
        parts.push('    </g>');
      });
    } else {
      // No families - output fills directly
      if (fillPaths.length > 0) {
        parts.push(`    <g id="layer-${safeId}-fills">`);
        parts.push(`      <!-- Fill paths (${fillPaths.length} paths) -->`);
        fillPaths.forEach(path => {
          if (!path.d) return;
          const escapedD = escapeXML(path.d);
          const stroke = path.stroke || 'black';
          parts.push(`      <path d="${escapedD}" fill="none" stroke="${stroke}" stroke-width="${path.strokeWidth || 0.1}" />`);
        });
        parts.push('    </g>');
      }
    }

    // Outlines within layer
    if (outlinePaths.length > 0) {
      parts.push(`    <g id="layer-${safeId}-outlines">`);
      parts.push(`      <!-- Outline paths (${outlinePaths.length} paths) -->`);
      outlinePaths.forEach(path => {
        if (!path.d) return;
        const escapedD = escapeXML(path.d);
        parts.push(`      <path d="${escapedD}" fill="none" stroke="${path.stroke || 'red'}" stroke-width="${path.strokeWidth || 0.1}" />`);
      });
      parts.push('    </g>');
    }
  ...
  // Output ungrouped paths (paths with no layer)
  if (ungroupedPaths.length > 0) {
    const fillPaths = ungroupedPaths.filter(p => !p.isOutline);
    const outlinePaths = ungroupedPaths.filter(p => p.isOutline);

    parts.push('  <g id="ungrouped">');
    parts.push(`    <!-- Ungrouped paths (${ungroupedPaths.length} paths) -->`);

    if (fillPaths.length > 0) {
      parts.push('    <g id="ungrouped-fills">');
      fillPaths.forEach(path => {
        if (!path.d) return;
        const escapedD = escapeXML(path.d);
        const stroke = path.stroke || 'black';
    ...
    if (outlinePaths.length > 0) {
      parts.push('    <g id="ungrouped-outlines">');
      outlinePaths.forEach(path => {
        if (!path.d) return;
        const escapedD = escapeXML(path.d);
        parts.push(`      <path d="${escapedD}" fill="none" stroke="${path.stroke || 'red'}" stroke-width="${path.strokeWidth || 0.1}" />`);
