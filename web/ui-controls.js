...
function buildSVGContent(paths) {
  ...
  // Add outline group if there are outlines
  if (outlinePaths.length > 0) {
    parts.push('  <g id="outlines">');
    parts.push(`    <!-- Outline strokes (${outlinePaths.length} paths) -->`);
...
function buildBinnedSVGContent(pathsWithMetadata, binCount) {
  ...
  bins.forEach((bin, binIndex) => {
    ...
    // Write all paths in this bin, separating outlines
    let pathCount = 0;
    bin.forEach(item => {
      item.paths.forEach(path => {
        ...
        // Separate outlines from fills
        if (path.class === 'outline') {
          allOutlines.push(path);
          return;
        }
  ...
  if (allOutlines.length > 0) {
    parts.push('  <g id="outlines">');
    parts.push(`    <!-- Outline strokes (${allOutlines.length} paths) -->`);

    allOutlines.forEach((path, index) => {
      if (!path.d || typeof path.d !== 'string') {
        return;
      }
